CREATE TABLE dim_year (
    ...
    publiction_year INT NOT NULL UNIQUE,
...
CREATE TABLE paper_fact (
    ...
    author_group_key SERIAL UNIQUE, -- could be in a separate dimension like in Kimdall's book but not sure why that is needed
    arxiv_ID VARCHAR(9) NOT NULL UNIQUE, -- NK
    doi VARCHAR(40) NOT NULL UNIQUE,
    title VARCHAR(200) NOT NULL,
    latest_version_nr VARCHAR(3),
    citaton_count INT,
    CONSTRAINT uq_fact_table UNIQUE(year_key, domain_key, type_key, venue_key, author_group_key),
    PRIMARY KEY(year_key, domain_key, type_key, venue_key, author_group_key) 
);
